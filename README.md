...
Pipeline:
Topic input --> Download transcripts for 1000 (or x) videos on that subject that were accepted via a comment rating criteria with NTLK --> 1000 (or x) number of transcripts are summarized in a divide by two tree structure, then reassembled at the end for a coherent output by an LLM --> Output then parsed into LaTeX by the LLM --> Presented to user.
